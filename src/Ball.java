...
public class Ball extends Circle
{
	...
	private boolean gameOver = false, restart = false;

	public Ball(Pane pane, Scene scene, Bricks bricks, Paddle paddle)
	{
		...
		if (!gameOver)
		{
			animation = new Timeline(new KeyFrame(Duration.millis(5),
					e -> moveBall(bricks, pane, scene, paddle)));
			animation.setCycleCount(Timeline.INDEFINITE);
			animation.play();
		}
	}

	protected void moveBall(Bricks bricks, Pane pane, Scene scene,
			Paddle paddle)
	{
		if ((x >= paddle.x && x <= paddle.x + 100) && y == pane.getHeight() - 10
				- paddle.getHeight() - ball.getRadius())
		{
			dy *= -1;
		} else if (y > 450)
		{
				...
				if (x + ball.getRadius() > 65 && x - ball.getRadius() < 520)
				{
									...
									+ bricks.yellow.get(i).getWidth())
					{
						if (y < bricks.yellow.get(i).getY()
								+ bricks.yellow.get(i).getHeight() && y > bricks.yellow.get(i).getY())
						{
			...
			for (int i = 0; i < bricks.orange.size(); ++i)
			{
				if (x + ball.getRadius() > 65 && x - ball.getRadius() < 520)
				{
					if (x > bricks.orange.get(i).getX()
							&& x < bricks.orange.get(i).getX()
									+ bricks.orange.get(i).getWidth())
					{
						if (y < bricks.orange.get(i).getY()
								+ bricks.orange.get(i).getHeight()
								&& y > bricks.orange.get(i).getY())
						{
							bricks.orange.get(i)
									.setHp(bricks.orange.get(i).hp - 1);
		...
		if (y - ball.getRadius() < 60 && y - ball.getRadius() > 40)
		{
			for (int i = 0; i < bricks.red.size(); ++i)
			{
				if (x + ball.getRadius() > 65 && x - ball.getRadius() < 520)
				{
					if (x > bricks.red.get(i).getX()
							&& x < bricks.red.get(i).getX()
									+ bricks.red.get(i).getWidth())
					{
						if (y < bricks.red.get(i).getY()
								+ bricks.red.get(i).getHeight()
								&& y > bricks.red.get(i).getY())
						{
							bricks.red.get(i)
									.setHp(bricks.red.get(i).hp - 1);
		...
		} else
		{
			...
			Text text = new Text(pane.getWidth() / 3, pane.getHeight() / 2,
					"Game Over");
